...
class RemoteConnection extends RemoteObservable {
    ...
    get authenticated() {
        return this._authenticationStatus === RemoteConnection.AuthenticationStatus.AUTHENTICATED;
    }
    ...
    send(message, persistent) {
        ...
        if (this.webSocketOpen
            && (this.authenticated || type === RemoteConnection.MessageTypes.AUTHENTICATION_CLIENT_SERVER_RESPONSE)) {
    ...
    async _answerServerChallenge(message) {
        ...
        this.send({
            type: RemoteConnection.MessageTypes.AUTHENTICATION_CLIENT_SERVER_RESPONSE,
            hash: clientServerHash.toBase64(),
            challenge: this._clientChallenge
        });
        this._authenticationStatus = RemoteConnection.AuthenticationStatus.WAITING_FOR_SERVER_RESPONSE;
    }
    ...
    async _checkServerResponse(message) {
        ...
        if (serverClientHash.toBase64() === message.data) {
            ...
            this._authenticationStatus = RemoteConnection.AuthenticationStatus.AUTHENTICATED;

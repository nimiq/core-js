...
class IWorker {
    ...
    static async startWorkerForProxy(clazz, name, workerScript) {
        if (!IWorker._workersSupported) {
            ...
            return IWorker.createProxy(clazz, name, new Worker(URL.createObjectURL(new Blob(
                [`Nimiq = {_path: '${Nimiq._path}', _avoidWasm: ${!!Nimiq._avoidWasm}}; importScripts('${workerScript.replace(/'/g, '')}');`]
            ))));
        }
    }

class MnemonicUtils {
    ...
    static _bytesToBinary(bytes) {
        return bytes.reduce((binary, byte) => binary + MnemonicUtils._lpad(byte.toString(2), '0', 8), '');
    }
    ...
    static keyToMnemonic(entropy, wordlist) {
        if (typeof entropy === 'string') entropy = MnemonicUtils._hexToArray(entropy);
        ...
        var entropyBits = MnemonicUtils._bytesToBinary(entropy);
        var checksumBits = MnemonicUtils._deriveChecksumBits(entropy);

        var bits = entropyBits + checksumBits;
        var chunks = bits.match(/(.{11})/g);
        var words = chunks.reduce((words, chunk) => {
            var index = MnemonicUtils._binaryToBytes(chunk);
    ...
    static mnemonicToKey(mnemonic, wordlist) {
        ...
        var newChecksum = MnemonicUtils._deriveChecksumBits(entropy).slice(0, checksumBits.length);
...
MnemonicUtils.HEX_ALPHABET = '0123456789abcdef';

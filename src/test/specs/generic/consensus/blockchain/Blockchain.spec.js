describe('Blockchain', () => {
    ...
    xit('will always verify a block before accepting it', (done) => {
        (async function () {
            ...
            status = await blockchain.pushBlock(block);
            expect(status).toBe(Blockchain.PUSH_ERR_INVALID_BLOCK);
            expect(Log.d).toHaveBeenCalledWith(Blockchain, 'Rejecting block, AccountsHash mismatch: current=ZFLBx3Lr7qAY1KnGOraKNGz7BTnHwrXD1DuLvi3w5sY=, block=R+pwzwiHK9tK+tNDKwHZY6x9Fl9rV1zXLvR0mPRFmpA=');
    ...
    xit('can store a block that starts a fork and switch when the fork becomes more secure', (done) => {
        (async function () {
            ...
            expect(status).toBe(Blockchain.PUSH_OK);
            expect(Log.v).toHaveBeenCalledWith(Blockchain, 'Found common ancestor AAAEg/ITvgDI5QOBxuCFYj0ngLxCWu0jjGzeJzp96Wc= 2 blocks up');

describe('Interlink LegacyV2', () => {
        ...
        (async () => {
            ...
            for (let i = 0; i < 5; i++) {
                const block = await bc.createBlock({
                    superblockLevel: 0,
                    version: BlockHeader.Version.LUNA_V2
                });
    ...
    it('is constructed correctly (constant level-0, increasing difficulty)', (done) => {
        (async () => {
            ...
            const interlinks = [[0], [1], [2], [3], [4], [5], [6], [7]];
    ...
    it('is constructed correctly (constant depth-1, increasing difficulty)', (done) => {
        (async () => {
            ...
            const interlinks = [[0], [1, 1], [2, 2], [3], [4], [5], [6], [7]];
    ...
    it('is constructed correctly (alternating depth 2, increasing difficulty)', (done) => {
        (async () => {
            const bc = await TestBlockchain.createVolatileTest(0, 5);

            // Push 8 blocks. The target depth increases between block 3,4 and 7,8.
            const levels = [2, 1, 0, 0, 2, 1, 0, 0];
            const interlinks = [[0], [1, 1, 1], [2, 2, 1], [3, 1], [4, 1], [5, 5, 5], [6, 6, 5], [7, 5]];
    ...
    it('is constructed correctly (constant level-0, alternating difficulty)', (done) => {
        (async () => {
            ...
            const interlinks = [[0], [1], [2], [3], [4, 4], [5], [6, 6], [7]];

            let ts = Block.GENESIS.timestamp;
            const hashes = [Block.GENESIS.HASH];
            for (let i = 0; i < interlinks.length; i++) {
                const block = await bc.createBlock({
                    superblockLevel: 0,

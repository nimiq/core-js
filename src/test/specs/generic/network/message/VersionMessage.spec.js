describe('VersionMessage', () => {
    ...
    it('is correctly constructed', () => {
        const msg1 = new VersionMessage(2, addr, Block.GENESIS.HASH, blockHash, challenge);
    ...
    it('must have well defined arguments', () => {
        expect(() => new VersionMessage(NumberUtils.UINT32_MAX+1, addr, Block.GENESIS.HASH, blockHash, challenge)).toThrow();
        expect(() => new VersionMessage(2, blockHash, Block.GENESIS.HASH, blockHash, challenge)).toThrow();
        expect(() => new VersionMessage(2, addr, addr, blockHash, challenge)).toThrow();
        expect(() => new VersionMessage(2, addr, Block.GENESIS.HASH, addr, challenge)).toThrow();
        expect(() => new VersionMessage(2, addr, Block.GENESIS.HASH, blockHash, addr)).toThrow();
    });
});
